...
import BlockAI from "../abis/BlockAI.json";
...
class App extends Component {
  ...
  setMintBtnTimer = () => {
    ...
    if (mintBtn !== undefined && mintBtn !== null) {
      ...
      this.state.lastMintTime === undefined || this.state.lastMintTime === null
        ? (mintBtn.innerHTML = "Mint BlockAI NFT")
  ...
  loadBlockchainData = async () => {
    ...
    if (accounts.length === 0) {
      ...
      const networkData = BlockAI.networks[networkId];
      if (networkData) {
        this.setState({ loading: true });
        const cryptoBoysContract = web3.eth.Contract(
          BlockAI.contracts["BlockAI.sol"]["BlockAI"].abi,
          networkData.address
        );
        ...
        let totalTokensMinted = await cryptoBoysContract.methods
          ...
          .call();
        console.log(JSON.stringify(totalTokensMinted));
  ...
  mintMyNFT = async (name, tokenPrice) => {
    ...
    if (!nameIsUsed) {
      ...
      this.setState({ loading: true });
      /**
       * Use AI Model to generate the art and get 
       */
      var raw = JSON.stringify(this.state.file);
      axios({
        ...
        let tokenURI = `https://dl-ipfs.dlunify.dltlabs.com/ipfs/${response.data.value[0].Hash}`;
        // let tokenURI = `https://dl-ipfs.dlunify.dltlabs.com/ipfs/`;
